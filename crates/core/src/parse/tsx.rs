...
pub fn parse_tsx<'a>(allocator: &'a Allocator, file_path: &PathBuf, file_content: &'a str) -> Result<(ParserReturn<'a>, SemanticBuilderReturn<'a>), String> {
    ...
    let named_source = NamedSource::new(file_path.to_string_lossy().to_string(), file_content.to_string());
