...
/// ```
pub fn resolve_file_path(dir: &PathBuf, specifier: &str, tsconfig: &PathBuf) -> Result<Resolution, String> {
  let options = ResolveOptions {
    ...
    extension_alias: vec![(".js".to_string(), vec![".ts".to_string(), ".js".to_string()])],
...
mod tests {
    ...
    fn test_resolve_file_path() {
      ...
      fs::write(&button_file, "export function Button() { return <div>Button</div>; }").unwrap();
      ...
      assert!(resolution.is_ok());
      assert_eq!(resolution.unwrap().path(), components_dir.join("Button.tsx"));
    }
    ...
    fn test_resolve_file_path_with_node_modules() {
      ...
      fs::write(&framer_motion_dir.join("index.js"), "module.exports = { motion: () => <div>Framer Motion</div> };").unwrap();
      ...
      assert!(resolution.is_ok());
      assert_eq!(resolution.unwrap().path(), node_modules_dir.join("framer-motion").join("index.js"));
    }
}
